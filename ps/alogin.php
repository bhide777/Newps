...
$susn = $_POST['usn'];
$stupass = $_POST['spsw'];
$sql = "SELECT 'usn','emial' FROM student WHERE usn='$susn' AND spass='$stupass'";
$result = mysqli_query($mysqli,$sql);
// $row = mysqli_num_rows($result);
if(mysqli_num_rows($result)==1){
    $_SESSION['s_usn']=$susn;
...
$fname = $_POST['fname'];
$fpass = $_POST['fpsw'];
$sql = "SELECT 'fname',fid FROM faculty WHERE fname='$fname' AND pass='$fpass'";
$result = mysqli_query($mysqli,$sql);
// $row = mysqli_num_rows($result);
if(mysqli_num_rows($result)==1){
    while($row=$result->fetch_assoc()){
    $_SESSION['fcid'] = $row['fid'];
    }
